/* overview_tree_store.c generated by valac 0.44.0, the Vala compiler
...
static gboolean __lambda4_ (Block49Data* _data49_,
                     GtkTreeModel* m,
                     GtkTreePath* p,
                     GtkTreeIter* i);
...
block49_data_unref (void * _userdata_)
{
	...
	if (g_atomic_int_dec_and_test (&_data49_->_ref_count_)) {
		valaDevelopOverviewTreeStore* self;
		self = _data49_->self;
		_g_free0 (_data49_->itemIter);
		_g_free0 (_data49_->full_path);
...
static gboolean
__lambda4_ (Block49Data* _data49_,
            GtkTreeModel* m,
            GtkTreePath* p,
            GtkTreeIter* i)
{
	...
	if (vala_develop_overview_tree_store_get_item_type (G_TYPE_CHECK_INSTANCE_CAST (m, VALA_DEVELOP_TYPE_OVERVIEW_TREE_STORE, valaDevelopOverviewTreeStore), &_tmp0_) == ((gint) VALA_DEVELOP_ITEM_TYPE_Source)) {
		...
		fullPath = _tmp2_;
		_tmp3_ = fullPath;
		if (g_strcmp0 (_tmp3_, _data49_->full_path) == 0) {
			GtkTreeIter _tmp4_;
			GtkTreeIter* _tmp5_;
			_tmp4_ = *i;
			_tmp5_ = __gtk_tree_iter_dup0 (&_tmp4_);
			_g_free0 (_data49_->itemIter);
			_data49_->itemIter = _tmp5_;
			result = TRUE;
			_g_free0 (fullPath);
			return result;
		}
                                                        ...
                                                        const gchar* full_path)
{
	...
	_g_free0 (_data49_->full_path);
	_data49_->full_path = _tmp0_;
	_data49_->itemIter = NULL;
	gtk_tree_model_foreach ((GtkTreeModel*) self, ___lambda4__gtk_tree_model_foreach_func, _data49_);
	_tmp1_ = __gtk_tree_iter_dup0 (_data49_->itemIter);
	result = _tmp1_;
