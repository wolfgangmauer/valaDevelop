/* rename_folder_dialog.c generated by valac 0.44.0, the Vala compiler
...
valaDevelopRename* vala_develop_rename_construct (GType object_type,
                                                  ...
                                                  GtkTreeIter* iter,
                                                  valaDevelopItemType itemType);
static void __lambda74_ (valaDevelopRename* self);
...
static void
__lambda74_ (valaDevelopRename* self)
{
	GtkButton* _tmp0_;
	GtkEntry* _tmp1_;
	const gchar* _tmp2_;
                               ...
                               valaDevelopItemType itemType)
{
	...
	_tmp4_ = _g_object_ref0 (treeStore);
	_g_object_unref0 (self->priv->_overviewTreeStore);
	self->priv->_overviewTreeStore = _tmp4_;
	_tmp5_ = *iter;
	self->priv->_overviewTreeIter = _tmp5_;
	self->priv->_itemType = itemType;
	_tmp6_ = self->priv->itemname;
	g_signal_connect_object ((GtkEditable*) _tmp6_, "changed", (GCallback) ___lambda74__gtk_editable_changed, self, 0);
	_tmp7_ = self->priv->_overviewTreeStore;
	_tmp8_ = self->priv->_overviewTreeIter;
	_tmp9_ = vala_develop_overview_tree_store_get_item_name (_tmp7_, &_tmp8_);
	_g_free0 (self->priv->_orgName);
	self->priv->_orgName = _tmp9_;
	_tmp10_ = self->priv->itemname;
	_tmp11_ = self->priv->_orgName;
...
vala_develop_rename_new (GtkWidget* parent,
                         ...
                         valaDevelopItemType itemType)
{
	return vala_develop_rename_construct (VALA_DEVELOP_TYPE_RENAME, parent, treeStore, iter, itemType);
}
	...
	g_return_if_fail (self != NULL);
	{
		...
		if ((_tmp5_ & VALA_DEVELOP_ITEM_TYPE_Directory) == VALA_DEVELOP_ITEM_TYPE_Directory) {
			...
			if (_tmp29_) {
				...
				GtkTreeIter* _tmp50_;
				GtkTreeIter _tmp51_;
				_tmp32_ = self->priv->_overviewTreeStore;
				...
				_tmp37_ = self->priv->_overviewTreeStore;
				...
				_g_free0 (_tmp45_);
				_tmp46_ = self->priv->_overviewTreeStore;
				_tmp47_ = self->priv->_overviewTreeStore;
			...
			GFile* _tmp74_;
			GFile* _tmp75_;
			_tmp52_ = self->priv->_overviewTreeStore;
			...
			if (_tmp73_) {
				...
				if (_tmp81_ == VALA_DEVELOP_ITEM_TYPE_Resource) {
					...
					gchar* _tmp92_;
					gchar* _tmp93_;
					_tmp82_ = self->priv->_overviewTreeStore;
					...
					_tmp85_ = self->priv->_overviewTreeStore;
				...
				_g_free0 (_tmp96_);
				cType = _tmp98_;
				_tmp99_ = self->priv->_overviewTreeStore;
				...
				_tmp139_ = self->priv->_overviewTreeStore;
				_tmp140_ = self->priv->_overviewTreeStore;
